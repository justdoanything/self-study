...
그리고 컨텐츠는 내용이 수정되거나 삭제될 수 있기 때문에 스크롤의 위치는 여러 상황에 따라 임의로 조정해줘야 합니다. 
...
2. onScrollTop과 onScrollBottom 구현합니다.

    store에는 일정한 데이터의 수만 저장하고 있고 스크롤의 onTop과 onBottom 이벤트를 감지해서 이전 데이터와 이후 데이터를 적절하게 가져옵니다.

    이렇게하면 사용자가 스크롤을 최상단으로 올렸을 때 처음 봤던 컨텐츠를 볼 수 있지만

    onTop의 페이징 사이즈와 num을 적절하게 조정해야함.
...
- 백화현상 : 데이터가 많아서 페이징이 로드되기 전에 이동될 경우 흰화면이 오래 뜨는 경우가 있다.  InView
