...
## 무한 스크롤 구현 고려사항
- 피드 하나의 높이와 중간에 끼어드는 피드의 높이는 될 수 있으면 고정 높이로하자. 높이가 가변적이면 뒤로가기에서 스크롤 위치를 잡을 때 어려움이 있다.
- 피드 컨텐츠에 이미지 등 용량이 많은 요소가 있다면 무한한 데이터를 RAM에 저장할 수 없기 때문에 되도록 이면 가벼운 컨텐츠를 사용하자. (플젝에선 데이터가 일정 수순 쌓이면 기존 데이터를 지웠다. 따라서 가능하다면 페이지 업/다운을 둘 다 구현하는것도 좋을 것 같다. 따라서 최상단/최하단도 스티키 헤더/푸터 때문에 구분을 잘 해야 한다.
- 신고하기 등으로 숨겨야할 경우, store에 있는 데이터를 잘 조작해야한다.
- 피드랑 중간에 추가 요소들이 들어갈 경우, store에 피드를 저장할 때 중간요소도 다 같이 포함해서 저장하는 것이 좋다.
- 피드 컨텐츠에 좋아요수, 댓글수, 조회수 등 실시간으로 반영되야하는 데이터가 있고 상세, 댓글 팝업 등 해당 데이터가 변경될 수 있음을 고려해야 한다. store에 있는 데이터에가 count를 -+1 하는데 실제 데이터를 해당 화면에서 fetch할 경우 수량이 안맞을 수 있다.
- 중간에 있는 피드 위치로 바로 가는 기능들이 구현하기 어려울 것 같다.
- 스크롤은 뒤로 가기 했을 때 페이지가 다 로딩되기 전에 스크롤 위치로 가버려서 잘못잡을 때가 있었다.
- 데이터가 많아서 페이징이 로드되기 전에 이동될 경우 흰화면이 오래 뜨는 경우가 있다.  InView
- URL 기준으로 store를 저장할 경우 같은 페이지 URL 내에서 여러 탭으로 되어있을 경우 로직이 복잡해질수있음.
  성능 이슈
- MySQL에서 타입이 다르면 CAST를 꼭 사용하자!! (mybatis를 쓰면 '' 로 묶이기 때문에 string이 기본적으로 사용된다.)
- 복잡한 쿼리의 경우 Id를 먼저 뽑아오고 두번째 쿼리에서 ORDER BY FIELD를 사용하자.
```sql
WHERE nc.notim_id IN
      <foreach collection="noticeMaterialIds" item="id" open="(" separator="," close=")">
         #{id}
      </foreach>
 ORDER BY FIELD(nc.notim_id,
      <foreach collection="noticeMaterialIds" item="id" separator=",">
         #{id}
      </foreach>
      )
```
공통컴포넌트를 만들 때 최하단 컴포넌트는 데이터를 만들어서 뿌리기만하고 이벤트 제어는 상단 컴포넌트에서 컨트롤하도록 하자.
회귀테스트 케이스를 최소한이라도 작성하면서 개발하자. 하나의 기능도 여러 SE가 돌아가면서 개발하고 리팩토링하고 버그수정을 하고 기획이 수시로 바뀌기 때문에 코드 수정을 하면서 기존 기능이 안되는 문제를 방지하기 위해서 SE가 개발 혹은 수정할 때 페이지 단위로 테스트케이스를 wiki에 작성하면서 공유하고 다른 SE가 해당 페이지를 수정 후 기능 테스트를 할 때 참고하도록 하면 좋을 것 같습니다. (테스트코드를 짜는게 제일 베스트지만... 시간상!), 데이터 없음, API 에러에 대한 케이스 정의 필요
Cache에 커뮤니티 폐쇄 등 실시간 데이터에 대한 고려
이미지 처리
- 썸네일 처리. 바로 파일 생성이 안되면? 
