...
- #### collect () - 부재 : Collectors
  ```java
  // groupingBy
  Map<Integer, List<Transaction>> transactionsByYears = transactions.stream().collec(Collectors.groupingBy(Transaction::getYear));
  ...
  // reduce를 이용한 값 비교
  System.out.println(menu.stream().collect(Collectors.reducing((d1, d2) -> d1.getCalorie() > d2.getCalories() ? d1 : d2)));
  System.out.println(menu.stream().reduce((d1, d2) -> d1.getCalories() > d2.getCalorie() ? d1 : d2));
  ```
