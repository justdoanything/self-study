...
EC2 10개 | CloudWatch 고유한 메트릭 이름, 사용자 지정 네임스페이스 |API G/W & Lambda | Session은 DynamoDB에 저장
...
S3에 업로드 하기 전아 파일이 암호화되었는지 확인 | GenerateDateKey API를 사용한 다음 해당 데이터 키를 사용해서 Lambda 함수 코드의 파일을 암호화
