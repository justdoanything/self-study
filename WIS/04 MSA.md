...
- ## 용어 정리
  ...
  - 분산처리
    ...
    - `CQRS` : Command Query Responsibility Segregation으로 여러 서비스에 종속되어 있는 데이터를 JOIN 해서 보여줘야하는 경우에 API Composition으로 처리할 수 있지만 CQRS 패턴으로 더 간단하게 처리할 수 있다. 한 서비스에 CUD가 발생하면 CUD 처리하고 내용을 Queue에 전달한다. CQRS 데이터베이스는 Queue를 읽으면서 데이터를 취합하고 UI는 CQRS 데이터베이스를 통해서 JOIN 데이터를 조회할 수 있다.
  - `Service Mesh` : 서비스 간의 통신을 제어하고 표시할 수 있도록 하는데 특화된 MSA을 위한 인프라 계층이다. Service Mesh를 구성하는 개별 proxy는 서비스 내부가 아니라 각 서비스와 함께 실행되므로 sidecar 라고도 부른다. sidecar proxy 들이 모여서 Mesh Network를 이룬다.
  - `Backing Service` : 데이터를 다루는 서비스로 데이터베이스와 캐시 시스템 등을 의미한다.
  - `Telemetry` : 서비스들을 모니터링하고 서비스별로 발생하는 이슈들을 대응할 수 있도록 환경을 구성하는 것이다. 모니터링, 로깅, 트래이싱이 있다.
  - `Outer Architecture` : North to South 구조. API Gateway, Service Mesh
  - `Inner Architecture` : West to East 구조. Container, Backing Service, CI/CD, Telemetry
