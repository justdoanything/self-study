...
- React로 구현하다가 부딪힌 점 
  - 깊은 복사, 얇은 복사
  - 페이지 저장할 때, 바뀐거만 저장할지, 전체를 저장할지 정해야한다.
  - 바뀐것만 정하면 모든 폼에서 변한 폼만 감지해서 변경감지해서 그것만 업데이트하는게 어려웠다.
  - 기본 값과 바뀐 값 전체를 1:1로 비교해서 바뀐것만 넘겨야함.
  - 상태관리에서 옵저버로 변화를 감지하는데 A, A-1 까지는 깊은복사가 됨. 새로운 주소가 따짐.
  - 근데 A-1을 props 로 던졌을 때 옵저버가 A-2의 변화를 감지하고 이니셜 벨류를 바꿔버림.
  - 그 이유는 주소를 따로 따지지만 proxy로 감싸져있기 때문에 옵저버가 A-3의 변화를 감지하고 수정된 값을 이니셜 값에 덮어버림.
  - 그럼 A에 대한 수정/비교가 일어났을때, 이니셜 값이 바꼈기 때문에 변화한 값으로 인지를 못함.
- 싱글페이지에서 변경된 값을 감지하는게 중요. 그래야 부분적으로 렌더링을 다시 해줘야 하니까.
